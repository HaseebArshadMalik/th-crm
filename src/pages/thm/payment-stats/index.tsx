  ...
  useEffect(() => {
    if (rowData) {
      ...
      const groupedData = rowData.reduce((acc, entry) => {
        ...
        let existingDay = acc.find((dayData: { name: any; }) => dayData.name === day);

        if (!existingDay) {
          existingDay = { name: day };
      ...
      setTransformedData(reversedGroupedData);
    }
  }, [rowData]);
      ...
      {transformedData.length > 0 && (
        <div className="boxShadow" style={{ overflow: "hidden" }}>
          <div style={{ padding: "40px 40px 40px 40px" }}>
            <MultiLineChart data={transformedData} />
          </div>
        </div>
      )}
...
export default ThmPaymentStats;
